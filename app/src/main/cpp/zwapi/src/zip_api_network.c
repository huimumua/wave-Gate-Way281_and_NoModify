...
#endif
    printf("**************Dump frame from node:%u, cmd:\n", (unsigned)src_node);
    debug_zwapi_bin_msg(&nw->plt_ctx, cmd_buf, cmd_len);
    printf("**************Dump frame from node done\n");
                                              ...
                                              uint16_t flag, uint16_t encap_fmt)
{
    ...
    if (appl_ctx->use_ipv4)
    {
        ...
        printf("==========> ipv4, zip_router addr: %s, sa.addr: %s\n", nw->zip_router_addr, inet_ntoa(sa4->sin_addr));
    ...
    printf("*********application_command_handler_cb, src_node:%d\n",src_node);
