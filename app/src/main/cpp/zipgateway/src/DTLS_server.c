...
void plt_bin_show_1(void * buf, uint32_t len)
{
    uint32_t    line_len;
    ...
    while (1)
    {
      ...
      dtls_session_timer_adjust();
          ...
          if(len>0)
          {
            ...
            DBG_PRINTF("===> recv buf from zware:\n");
            plt_bin_show_1(read_buf, len);
            DBG_PRINTF("===> recv buf from zware end\n");

            if(node == MyNodeID)
            {
              DBG_PRINTF("linhui 123 to zipgateway\n");
              UDPCommandHandler(c,read_buf, len,TRUE);
            }
            else
            {
              DBG_PRINTF("linhui 124 to other\n");
